...
import { GetActiveUserActionResult, IUserToEdit, IUserPermission } from '@/app/(main)/users/types';
...
export async function getActiveUserByIdAction(
  userId: string
): Promise<GetActiveUserActionResult> {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const formattedUser: IUserToEdit = {
        ...
        companyIds: user.employee.companies.map((company) => company.companyId),
      };
